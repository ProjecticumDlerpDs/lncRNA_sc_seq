...
e85_seuratobject_10 <- FindClusters(e85_seuratobject_10, resolution = 0.5)
e85_seuratobject_20 <- FindClusters(e85_seuratobject_20, resolution = 0.5)
e85_seuratobject_30 <- FindClusters(e85_seuratobject_30, resolution = 0.5)
...
all.markers_10 <- FindAllMarkers(object = e85_seuratobject_10, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
all.markers_20 <- FindAllMarkers(object = e85_seuratobject_20, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
all.markers_30 <- FindAllMarkers(object = e85_seuratobject_30, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)

# output bestand maken
write_xlsx(all.markers_10, file.path(data_out, "markers_10.xlsx"))
write_xlsx(all.markers_20, file.path(data_out, "markers_20.xlsx"))
write_xlsx(all.markers_30, file.path(data_out, "markers_30.xlsx"))

# Het filteren op de top 10 markers per PC. 
top10_10 <- all.markers_10 %>% 
  group_by(cluster) %>% 
  slice_max(order_by = avg_log2FC, n = 10)

top10_20 <- all.markers_20 %>% 
  group_by(cluster) %>% 
  slice_max(order_by = avg_log2FC, n = 10)

top10_30 <- all.markers_30 %>% 
  group_by(cluster) %>% 
  slice_max(order_by = avg_log2FC, n = 10)

# exproteren van de top 10 markers.
write_xlsx(top10_10, file.path(data_out, "top10_10.xlsx"))
write_xlsx(top10_20, file.path(data_out, "top10_20.xlsx"))
write_xlsx(top10_30, file.path(data_out, "top10_30.xlsx"))

# Het filteren van welke genen dubbel zijn in de clusters.
dubbele_genen_10 <- top10_10 %>%
  group_by(gene) %>%
  summarize(
