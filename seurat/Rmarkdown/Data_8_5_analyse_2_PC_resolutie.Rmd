...
```{r dimentie bepalen van dataset, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r markers vinden van PC 10, message=FALSE, warning=FALSE, eval=FALSE, echo=FALSE}

all.markers_10 <- FindAllMarkers(object = e85_seuratobject_10, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)

```

```{r markers vinden van PC 20, message=FALSE, warning=FALSE, eval=FALSE, echo=FALSE}

all.markers_20 <- FindAllMarkers(object = e85_seuratobject_20, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)

```

```{r markers vinden van PC 30, message=FALSE, warning=FALSE, eval=FALSE, echo=FALSE}
...
```{r top 10 markers per cluster, warning=FALSE, message=FALSE, eval=FALSE, echo=FALSE}
...
```{r lijst dubbele markers, eval=FALSE, message=FALSE, warning=FALSE, echo=FALSE}
duplicated_genes_10 <- top10_10 %>%
  group_by(gene) %>%
  summarize(
...
```{r resolutie testen met clustree, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE, results='hide'}
e85_seuratobject_res <- FindNeighbors(e85_seuratobject, dims = 1:30)

e85_seuratobject_res_1 <- FindClusters(e85_seuratobject_res, resolution = seq(0.0, 1.0, 0.2))

e85_seuratobject_res_2 <- FindClusters(e85_seuratobject_res, resolution = seq(0.5, 1.0, 0.1))

e85_seuratobject_res_3 <- FindClusters(e85_seuratobject_res, resolution = seq(1.0, 1.5, 0.1))

e85_seuratobject_res_4 <- FindClusters(e85_seuratobject_res, resolution = seq(1.5, 2.0, 0.1))

clustree(e85_seuratobject_res_1, prefix = "RNA_snn_res.") + 
  theme(legend.position = "bottom") + 
  guides(edge_colour = FALSE, edge_alpha = FALSE)

clustree(e85_seuratobject_res_2, prefix = "RNA_snn_res.") + 
  theme(legend.position = "bottom") + 
  guides(edge_colour = FALSE, edge_alpha = FALSE)

clustree(e85_seuratobject_res_3, prefix = "RNA_snn_res.") + 
  theme(legend.position = "bottom") + 
  guides(edge_colour = FALSE, edge_alpha = FALSE)

clustree(e85_seuratobject_res_4, prefix = "RNA_snn_res.") + 
  theme(legend.position = "bottom") + 
  guides(edge_colour = FALSE, edge_alpha = FALSE)
